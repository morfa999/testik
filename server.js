...
async function initDB() {
  ...
  try {
    ...
    // Добавить столбец plays если его нет
    try { await c.query(`ALTER TABLE sounds ADD COLUMN IF NOT EXISTS plays INT NOT NULL DEFAULT 0`); } catch {}
...
app.post('/api/reports', async (req,res) => {
  try { const u=await getUser(req); if(!u) return res.json({ok:false,error:'Не авторизован'});
  const {message}=req.body; if(!message?.trim()) return res.json({ok:false,error:'Введите сообщение'});
  const id=genId();
...
app.get('/api/broadcasts', async (req,res) => {
  try { const u=await getUser(req); if(!u) return res.json({broadcasts:[],unread:0});
  ...
  res.json({broadcasts,unread}); } catch(e){console.error(e);res.json({broadcasts:[],unread:0});}
});

app.post('/api/broadcasts/mark-read', async (req,res) => {
  try { const u=await getUser(req); if(!u) return res.json({ok:false});
...
app.get('/api/admin/users', async (req,res) => {
  ...
  res.json(r.rows.map(u=>({id:u.id,name:u.name,email:u.email,avatarColor:u.avatar_color,subscription:u.subscription,isAdmin:u.is_admin,createdAt:u.created_at}))); } catch{res.json([]);}
});

app.get('/api/admin/reports', async (req,res) => {
  try { const u=await getUser(req); if(!isAdmin(u)) return res.json([]);
  const r=await pool.query(`SELECT * FROM reports WHERE status='new' ORDER BY created_at DESC`);
  res.json(r.rows); } catch{res.json([]);}
});
...
app.post('/api/admin/broadcasts', async (req,res) => {
  try { const u=await getUser(req); if(!isAdmin(u)) return res.json({ok:false});
  const {title,body}=req.body; if(!title?.trim()||!body?.trim()) return res.json({ok:false});
  const id=genId();
