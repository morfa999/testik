...
import { CloseIcon, WaveformIcon, PackageIcon } from './Icons';
...
const AddModal: React.FC<AddModalProps> = ({ isOpen, onClose, onAddSound, onAddPack }) => {
  const [mode, setMode] = useState<'sound' | 'pack'>('sound');
  ...
  const [packIsFree, setPackIsFree] = useState(true);
  const [packFiles, setPackFiles] = useState<File[]>([]);
  const [error, setError] = useState('');
  const fileInputRef = useRef<HTMLInputElement>(null);
  const packFilesRef = useRef<HTMLInputElement>(null);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      audio.onloadedmetadata = () => {
        ...
        // Проверка максимум 5 минут (300 секунд)
        if (s > 300) {
  ...
  const handleSubmitPack = () => {
    setError('');
    if (!packTitle.trim()) { setError('Введите название пака'); return; }
    if (packFiles.length === 0) { setError('Загрузите файлы'); return; }
    onAddPack({ title: packTitle.trim(), soundCount: packFiles.length, category: 'Pack', isFree: packIsFree });
    resetForm(); onClose();
  };

  const resetForm = () => { setTitle(''); setCategory(soundCategories[0]); setTags(''); setIsFree(true); setFileName(''); setFileData(undefined); setAudioDuration('0:00'); setAudioDurationSecs(0); setPackTitle(''); setPackFiles([]); setError(''); };
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 overflow-y-auto">
        ...
        <div className="relative w-full max-w-md bg-white rounded-2xl border border-[#E5E5E5] shadow-lg animate-scale-in p-6 my-4">
          ...
          <div className="flex gap-6 mb-5 border-b border-[#EBEBEB]">
            <button onClick={() => setMode('sound')} className={`relative flex items-center gap-1.5 pb-3 text-[13px] font-medium transition-all ${mode === 'sound' ? 'text-[#0A0A0A]' : 'text-[#999] hover:text-[#6B6B6B]'}`}>
              <WaveformIcon size={14} />Звук{mode === 'sound' && <span className="absolute bottom-0 left-0 right-0 h-[2px] bg-[#0A0A0A] rounded-full" />}
            </button>
            <button onClick={() => setMode('pack')} className={`relative flex items-center gap-1.5 pb-3 text-[13px] font-medium transition-all ${mode === 'pack' ? 'text-[#0A0A0A]' : 'text-[#999] hover:text-[#6B6B6B]'}`}>
              <PackageIcon size={14} />Пак{mode === 'pack' && <span className="absolute bottom-0 left-0 right-0 h-[2px] bg-[#0A0A0A] rounded-full" />}
            </button>
          </div>
          ...
          {mode === 'sound' ? (
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="text-[11px] font-semibold text-[#999] uppercase tracking-wider mb-1 block">Название пака *</label>
                <input type="text" placeholder="Trap Essentials Vol.1" value={packTitle} onChange={(e) => setPackTitle(e.target.value)} className="w-full px-4 py-2.5 bg-[#F8F8F8] border border-[#E5E5E5] rounded-xl text-[13px] text-[#0A0A0A] placeholder:text-[#C0C0C0] focus:outline-none focus:border-[#0A0A0A] transition-all" />
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => setPackIsFree(true)} className={`flex-1 px-4 py-2 rounded-lg text-[12px] font-semibold transition-all ${packIsFree ? 'bg-[#0A0A0A] text-white' : 'bg-[#F3F3F3] text-[#6B6B6B]'}`}>Бесплатный</button>
                <button onClick={() => setPackIsFree(false)} className={`flex-1 px-4 py-2 rounded-lg text-[12px] font-semibold transition-all ${!packIsFree ? 'bg-[#0A0A0A] text-white' : 'bg-[#F3F3F3] text-[#6B6B6B]'}`}>Premium</button>
              </div>
              <button onClick={handleSubmitPack} className="w-full mt-2 py-3 bg-[#0A0A0A] text-white text-[13px] font-semibold rounded-xl hover:bg-[#1A1A1A] transition-all active:scale-[0.98]">Добавить пак</button>
            </div>
          )}
        </div>
      </div>
